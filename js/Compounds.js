AFRAME.registerComponent("atoms", {
  ...
  createAtoms: async function (element) {
    ...
    for(var num = 1; num<=numOfElectron; num++){
      ...
      orbit.setAttribute("rotation", {x:0, y:orbitA, z:0});
      orbitA +=45;
      atom.appendChild(orbit);

      var eg = document.createElement("a-entity");
      eg.setAttribute("id", `electron-group-${elementName}`);
      eg.setAttribute("rotation", {x:0, y:0, z:electronA});
      electronA += 65;
      eg.setAttribute("animation", {
        property:"rotation",
        to:`0 0 -360`,
        loop:"true",
        dur:3500,
      ...
      electron.setAttribute("position", {x:0.2, y:0.2, x:0});
      eg.appendChild(electron);
